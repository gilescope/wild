...
fn compare_aggregate() {
    ...
    for path in &files {
        ...
        let wilt_len = if validates(&wilt_out) {
            Some(wilt_out.len())
        } else {
            None
        };
        ...
        let (wilt_hint_out, wilt_hint_ms) =
            if let Some(hints) = wilt::linker_hints::DerivedHints::from_bytes(&bytes) {
                let t0 = Instant::now();
                let out = wilt::optimise_with_hints(&bytes, &hints);
                let ms = t0.elapsed().as_millis();
                let len = if validates(&out) {
                    Some(out.len())
                } else {
                    None
                };
        ...
        let wilt_out_gz = if wilt_len.is_some() {
            gzip_size(&wilt_out)
        } else {
            0
        };
        ...
        let opt_out_gz = if opt_out.is_some() {
            ...
            if let Some((b, _)) = run_wasm_opt(path, "-O") {
                gzip_size(&b)
            } else {
                0
            }
    ...
    println!(
        "files compared:    {} of {} (skipped invalid: {}, wasm-opt failed: {})",
        both,
        files.len(),
        n_skipped_invalid,
        n_opt_failed
    ...
    println!(
        "wall time: wilt {} ms / wilt+hints {} ms / wasm-opt {} ms",
