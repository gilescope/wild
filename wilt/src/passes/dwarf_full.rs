...
//! - `Full`   : Lines + preserve-or-rewrite the rest of the DWARF sections that reference code
//!   addresses.
//!
//! Implementation staged (mirrors Phase 2b's structure):
//!
//! - **Step 1**: preserve-if-accurate. When the code section is byte-identical to the input's AND
//!   the FuncRemap is identity, every input `.debug_*` section is still correct — preserve them
//!   verbatim.
//! - **Step 2**: per-function byte-level address patching of the simpler address-carrying sections
//!   — `.debug_ranges` and `.debug_loc` (DWARF 4 fixed formats) — for the "bodies byte- identical,
//!   some moved" case.
//! - **Step 3**: add `.debug_aranges` patching.
//! - **Step 4**: abbrev-driven DIE walker for `.debug_info`.
//! - **Step 5**: DWARF-5 long tail — `.debug_rnglists`, `.debug_loclists`, `.debug_addr`.
//! - **Step 6** (this commit): honour DWARF-64 faithfully — detect the `unit_length == 0xFFFFFFFF`
//!   escape, widen offset reads to u64, thread the flag through every form skipper and header
//!   parser. Addresses stay `address_size` (4 bytes for wasm) regardless of the 32/64 flag — only
//!   SECTION offsets widen. Eliminates the "silent strip on DWARF-64 input" bug. A follow-up will
//!   add DWARF-64 → DWARF-32 downconversion when offsets fit in u32 (the common case for wasm).
//!
...
pub fn preserve_full_debug(
    ...
    let shifts = build_shifts(remap, &in_offsets, &out_offsets, input, optimised)?;
...
fn collect_debug_sections(
    ...
    for s in m.sections() {
        ...
        let Some(name_span) = s.custom_name else {
            continue;
        };
        ...
        let Some((nlen, c)) = crate::leb128::read_u32(p) else {
            continue;
        };
...
fn collect_named_section(m: &WasmModule<'_>, target: &str) -> Option<Vec<u8>> {
    ...
    for s in m.sections() {
        ...
        if name_span.slice(data) != target.as_bytes() {
            continue;
        }
        let p = s.payload.slice(data);
...
fn skip_form(bytes: &[u8], form: u64, address_size: u8, dwarf_64: bool) -> Option<usize> {
    ...
    Some(match form {
        ...
        DW_FORM_DATA1 | DW_FORM_FLAG | DW_FORM_REF1 | DW_FORM_STRX1 | DW_FORM_ADDRX1 => 1,
        DW_FORM_DATA2 | DW_FORM_REF2 | DW_FORM_STRX2 | DW_FORM_ADDRX2 => 2,
        DW_FORM_STRX3 | DW_FORM_ADDRX3 => 3,
        DW_FORM_DATA4 | DW_FORM_REF4 | DW_FORM_STRX4 | DW_FORM_ADDRX4 | DW_FORM_REF_SUP4 => 4,
        ...
        | DW_FORM_RNGLISTX => read_uleb(bytes)?.1,
        DW_FORM_STRP | DW_FORM_REF_ADDR | DW_FORM_SEC_OFFSET | DW_FORM_STRP_SUP
        | DW_FORM_LINE_STRP => off_size,
    ...
    while off < out.len() {
        ...
        while off < cu_end {
            ...
            for attr in &abbrev_decl.attrs {
                ...
                let attr_off = off;
                let consumed = skip_form(out.get(attr_off..)?, attr.form, address_size, dwarf_64)?;
                let is_address_attr = matches!(attr.name, DW_AT_LOW_PC | DW_AT_ENTRY_PC)
                    || (attr.name == DW_AT_HIGH_PC && attr.form == DW_FORM_ADDR);
                if is_address_attr && attr.form == DW_FORM_ADDR && consumed == address_size as usize
                {
                    let addr = u32::from_le_bytes(out[attr_off..attr_off + 4].try_into().ok()?);
                    if let Some(d) = shifts
                        .iter()
                        .find(|((s, e), _)| addr >= *s && addr < *e)
                        .map(|(_, d)| *d)
                    {
                        let new_addr = (addr as i64 + d).max(0) as u32;
                        out[attr_off..attr_off + 4].copy_from_slice(&new_addr.to_le_bytes());
                    }
                }
                off += consumed;
            }
...
fn offset_size(dwarf_64: bool) -> usize {
...
fn patch_debug_rnglists(body: &[u8], shifts: &[((u32, u32), i64)]) -> Option<Vec<u8>> {
    let mut out = body.to_vec();
    let mut off = 0;
    while off < out.len() {
        let (unit_length, dwarf_64, ul_consumed) = read_unit_length(&out[off..])?;
        ...
        let offset_entry_count =
            u32::from_le_bytes(out[header_pos + 4..header_pos + 8].try_into().ok()?) as usize;
        if address_size != 4 {
            return None;
        }
...
fn patch_debug_loclists(body: &[u8], shifts: &[((u32, u32), i64)]) -> Option<Vec<u8>> {
    let mut out = body.to_vec();
    let mut off = 0;
    while off < out.len() {
        let (unit_length, dwarf_64, ul_consumed) = read_unit_length(&out[off..])?;
...
fn patch_debug_addr(body: &[u8], shifts: &[((u32, u32), i64)]) -> Option<Vec<u8>> {
    let mut out = body.to_vec();
    let mut off = 0;
    while off < out.len() {
...
/// doesn't emit it.
fn patch_debug_aranges(body: &[u8], shifts: &[((u32, u32), i64)]) -> Option<Vec<u8>> {
    let mut out = body.to_vec();
    let mut off = 0;
...
fn patch_debug_loc(body: &[u8], shifts: &[((u32, u32), i64)]) -> Option<Vec<u8>> {
...
fn is_debug_section(name: &str) -> bool {
    CODE_REFERENCING.contains(&name) || NON_ADDRESS.contains(&name) || name.starts_with(".debug_")
}
...
mod tests {
    ...
    fn unchanged_module_preserves_all_debug_sections() {
        ...
        let names: Vec<&str> = preserved.sections.iter().map(|(n, _)| n.as_str()).collect();
    ...
    fn patch_debug_info_shifts_low_pc_addr() {
        ...
        // - address_size: u8 = 4
        // - DIE: abbrev_code (uleb 1), low_pc (4 bytes = 100), high_pc (4 bytes = 50),
        //   end-of-children (0).
        ...
        let new_low_pc =
            u32::from_le_bytes(patched[low_pc_off..low_pc_off + 4].try_into().unwrap());
        ...
        let new_high_pc =
            u32::from_le_bytes(patched[high_pc_off..high_pc_off + 4].try_into().unwrap());
    ...
    fn patch_debug_info_handles_high_pc_as_addr() {
        ...
        let off = 4 + 2 + 4 + 1 + 1;
        assert_eq!(
            u32::from_le_bytes(patched[off..off + 4].try_into().unwrap()),
            150
        );
        assert_eq!(
            u32::from_le_bytes(patched[off + 4..off + 8].try_into().unwrap()),
            200
        );
    }
