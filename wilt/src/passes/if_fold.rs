...
//!
//! Scope:
//!   * Void blocktype (0x40) only. Non-void would need to preserve value-producing stack effect;
//!     `drop; A` changes nothing there but we keep the bar simple.
//!   * Requires an explicit `else` arm (no elided-else form — there's nothing to dedupe).
//!   * Bails if either arm contains `br`/`br_if`/`br_table` — removing the if-frame shifts every
//!     label depth by one and the fold would have to renumber. Deferred.
...
fn rewrite_body_with_edits(body: &[u8]) -> Option<(Vec<u8>, crate::provenance::BodyEdits)> {
    ...
    for i in 0..n {
        ...
        if body.get(it.start as usize + 1).copied() != Some(0x40) {
            continue;
        }

        let Some(&end_idx) = ends.get(&i) else {
            continue;
        };
        let Some(&else_idx) = elses.get(&i) else {
            continue;
        };
        ...
        if range_contains_branch(&ir, i + 1, else_idx) {
            continue;
        }
    ...
    for &(from, to, keep) in &edits {
        ...
        body_edits.push(
            crate::provenance::Edit::subst(from as u32, (to - from) as u32, out_start, out_len),
            None,
        );
        cursor = to;
    }
...
mod tests {
    ...
    fn folds_identical_branches() {
        ...
        let body = [
            1, 1, 0x7F, 0x20, 0, 0x04, 0x40, 0x01, // then: nop
            0x05, // else
            0x01, // else: nop
            0x0B, // end if
            0x0B, // end func
    ...
    fn leaves_differing_branches() {
        let body = [
            1, 1, 0x7F, 0x20, 0, 0x04, 0x40, 0x01, 0x05, 0x01, 0x01, // else has extra nop
            0x0B, 0x0B,
        ];
    ...
    fn bails_on_branch_in_arm() {
        ...
        let body = [
            1, 1, 0x7F, 0x20, 0, 0x04, 0x40, 0x0C, 0x00, 0x05, 0x0C, 0x00, 0x0B, 0x0B,
        ];
        assert!(rewrite_body(&body).is_none());
    }
    ...
    fn leaves_non_void_blocktype() {
        let body = [
            1, 1, 0x7F, 0x20, 0, 0x04, 0x7F, // if (result i32)
            0x41, 1, 0x05, 0x41, 1, 0x0B, 0x1A, 0x0B,
        ];
        assert!(rewrite_body(&body).is_none());
    }
    ...
    fn leaves_missing_else() {
        let body = [1, 1, 0x7F, 0x20, 0, 0x04, 0x40, 0x01, 0x0B, 0x0B];
        assert!(rewrite_body(&body).is_none());
    }
}
