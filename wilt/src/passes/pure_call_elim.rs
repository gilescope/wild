...
//!
//! Out of scope:
//!   * Pure calls whose results are *consumed* (would need constant substitution, i.e. real IPA).
//!   * Impure calls with dropped results (unsound — side effects still have to run).
...
fn rewrite_body_with_edits(
    ...
    while off < body.len() {
        ...
        let Some(&(n, k)) = arities.get(&f) else {
            off += len;
            continue;
        };
...
fn compute_pure_arities(
    ...
    for (i, &tidx) in func_types.iter().enumerate() {
        ...
        let Some(&arity) = type_arities.get(tidx as usize) else {
            continue;
        };
    ...
    for abs in 0..num_imports {
        ...
        let Some(tidx) = import_func_type_idx(module, abs) else {
            continue;
        };
        let Some(&arity) = type_arities.get(tidx as usize) else {
            continue;
