...
fn rewrite_body_with_edits(body: &[u8]) -> Option<(Vec<u8>, crate::provenance::BodyEdits)> {
    ...
    for i in 0..n {
        ...
        if prev.op != OP_I32_CONST {
            continue;
        }
        let Some((cond, _)) = leb128::read_i32(&body[(prev.start + 1) as usize..]) else {
            continue;
        };
        let truthy = cond != 0;

        let end_idx = match ends.get(&i) {
            Some(&e) => e,
            None => continue,
        };
        ...
        if range_contains_branch(&ir, chosen_start, chosen_end) {
            continue;
        }
...
mod tests {
    ...
    fn leaves_non_empty_blocktype_alone() {
        ...
        let body = [
            0u8, 0x41, 1, 0x04, 0x7F, // if (result i32)
            0x41, 5, 0x05, 0x41, 9, 0x0B, 0x1A, 0x0B,
        ];
