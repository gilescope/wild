...
//!
//! Computed:
//! - `internal`: defined funcs not exported, not in element segments, not appearing as a `ref.func`
//!   target anywhere, not the start function. Anything in those sets is potentially reachable from
//!   outside the module's call graph.
//! - `call_count`: per-defined-func count of `call N` references across every body.
//! - `tables`: function indices stored in each (active) element segment. Indexed by the segment's
//!   table index.
//! - `ref_func_targets`: union of every `ref.func N` target across bodies, element segments, and
//!   global init exprs.
//! - `global_is_read`: false iff `global.get g` never appears in any body or any init expression.
//!
...
fn collect_global_consts(
    module: &WasmModule<'_>,
    first_defined_idx: u32,
) -> HashMap<u32, ConstVal> {
    let mut out = HashMap::new();
    let Some(sec) = module.section(wmod::SECTION_GLOBAL) else {
        return out;
    };
...
fn collect_exported_func_indices(module: &WasmModule<'_>) -> Vec<u32> {
    let mut out = Vec::new();
    let Some(sec) = module.section(wmod::SECTION_EXPORT) else {
        return out;
    };
    let p = sec.payload.slice(module.data());
    let (count, mut off) = match leb128::read_u32(p) {
        Some(x) => x,
        None => return out,
    };
    for _ in 0..count {
        let Some((nl, c)) = leb128::read_u32(p.get(off..).unwrap_or(&[])) else {
            break;
        };
        off += c + nl as usize;
        if off >= p.len() {
        ...
        let kind = p[off];
        off += 1;
        let Some((idx, c)) = leb128::read_u32(p.get(off..).unwrap_or(&[])) else {
            break;
        };
        off += c;
        if kind == 0x00 {
...
fn collect_element_func_refs(
    ...
    for _ in 0..count {
        let Some((flags, c)) = leb128::read_u32(payload.get(off..).unwrap_or(&[])) else {
            return;
        };
        off += c;
        let table_idx = match flags {
            0 => 0u32,
            2 => {
                let Some((t, c)) = leb128::read_u32(payload.get(off..).unwrap_or(&[])) else {
                    return;
                };
                off += c;
                t
        ...
        let Some(end_offset) = skip_const_expr(payload, off) else {
            return;
        };
        off = end_offset;
        let Some((n, c)) = leb128::read_u32(payload.get(off..).unwrap_or(&[])) else {
            return;
        };
        off += c;
        for _ in 0..n {
            let Some((f, c)) = leb128::read_u32(payload.get(off..).unwrap_or(&[])) else {
                return;
            };
            off += c;
            ref_funcs.insert(f);
...
fn collect_global_init_ref_funcs(payload: &[u8], out: &mut HashSet<u32>) {
    ...
    for _ in 0..count {
        ...
        let Some(end) = scan_const_expr_for_ref_func(payload, off, out) else {
            return;
        };
        off = end;
    }
